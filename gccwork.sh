...
curl "https://gcc.gnu.org/git/?p=gcc.git&a=search&h=HEAD&st=committer&s=saaadhu" | pup 'td a[class="list subject"] json{}' | jq 'map(@html "[\(.text)](https://gcc.gnu.org/git\(.href))")' | tail -n +2 | head -n -2 | sed -e 's/"//g' | sed -e 's/,$//g' | sed -e 'G;' >> $FILENAME 
